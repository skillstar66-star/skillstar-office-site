...
const testimonials = [
  {
    id: 1,
    name: 'Marcus Thorne',
    role: 'Director, Clawde Real Estate',
    quote: '"The build quality impressed dramatically. Before SkillStar, we had a lot of paid leads. Now, our sales team is closing deals at a record pace."',
  ...
  },
  {
    ...
    quote: '"Their team is proactive, creative, and result-driven. We\'ve seen a 280% growth in leads within just 6 months!"',
  ...
  },
  {
    ...
    quote: '"From SEO to paid ads, everything was handled perfectly. Our brand visibility and ROI have never been better."',
...
const trustedBrands = [
  { name: 'Clawde Real Estate', icon: Hexagon },
  { name: 'zenith', icon: Triangle },
  { name: 'brightscout', icon: Command },
  { name: 'NEXORA', icon: Boxes },
  { name: 'Pulse Digital', icon: Flame },
  { name: 'Vertex', icon: Code },
];
            ...
            <div className="flex items-center gap-6 mb-12">
              <div className="flex -space-x-3">
                {avatars.map((avatar, i) => (
                  <img key={i} src={avatar} alt="Client" className="w-12 h-12 rounded-full border-2 border-[#02040f] object-cover relative" style={{ zIndex: 3 - i }} />
                ))}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <img src={currentTest.avatar} alt={currentTest.name} className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/20 object-cover" />
                    <div>
        ...
        <div className="bg-[#050711] border border-white/5 rounded-2xl py-6 md:py-8 px-6 md:px-12 flex flex-col items-center gap-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-wrap justify-center md:justify-between items-center gap-6 md:gap-8 w-full opacity-60">
            {trustedBrands.map((brand, i) => (
              <div key={i} className="flex items-center gap-2 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-300 cursor-pointer">
                <brand.icon className="w-5 h-5 md:w-6 md:h-6 text-white" />
                <span className="text-white font-bold font-heading text-base md:text-xl tracking-tight">{brand.name}</span>
              </div>
            ))}
          </div>
        </div>
